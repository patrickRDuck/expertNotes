...
interface newNoteCardProps {
    onNoteCreate: (contet: string) => void
}
...
export function NewNoteCard({onNoteCreate}: newNoteCardProps) {
    ...
    function handleStartRecording() {
        ...
        if(!isSpeechRecognitionAPIAvalible) {
            alert('Infelimente o seu navegador não suporta o recurso de gravação')
    ...
    return(
        <Dialog.Root>
            <Dialog.Trigger className='rounded-md outline-none flex flex-col bg-slate-700 p-5 gap-3 text-left overflow-hidden hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className="overflow-hidden fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
                    <Dialog.Close className="absolute right-0 top-0 bg-transparent p-1.5 text-slate-400 hover:text-slate-100">
                        <X className="size-5"/>
                    </Dialog.Close>
...

                        {
                            isRecording ? (
                                <button 
                                 ...
                                 className="w-full bg-slate-900 py-4 flex items-center justify-center gap-2 text-center text-sm text-slate-300 outline-none hover:text-slate-100"
                                >
                                ...
                                <button 
                                 ...
                                 className="w-full bg-lime-400 py-4 text-center text-sm text-slate-950 outline-none hover:bg-lime-500"
                                >
                                    Salvar nota
                                </button>
                            )
